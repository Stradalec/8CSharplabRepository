...
namespace Lab8WinForms
{
    ...
    class Model 
    {
        ...
        public List<string> SyncDirectories(string firstDirectory, string secondDirectory)
        {
            ...
            if (!isInverted)
            {
                resultofSynchronize = SyncDirectory(mainDirectoryInfo, targetDirectoryInfo);
                return resultofSynchronize;
            }
            else
            {
                resultofSynchronize = SyncDirectory(targetDirectoryInfo , mainDirectoryInfo);
                return resultofSynchronize;
            }
            
        }

        private List<string> SyncDirectory(DirectoryInfo firstDirectoryInfo, DirectoryInfo targetDirectoryInfo )
        {
            ...
            foreach (FileInfo directoryFile in firstDirectoryInfo.GetFiles())
            {
            ...
            {
                FileInfo fileInMainDirectory = new FileInfo(Path.Combine(firstDirectoryInfo.FullName, directoryFile.Name));
    ...
    class Presenter 
    {
        ...
        {
            List<string> resultOfSynchronization = model.SyncDirectories(mainView.FirstDirectory(),mainView.SecondDirectory());
